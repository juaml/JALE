...
def load_excel(filepath, type='analysis'):
    ...
    if type == 'experiment':
        ...
        current_column_names[:6] = ['Articles',
                                    'Subjects', 'x', 'y', 'z', 'CoordinateSpace']
...
def concat_coordinates(exp_info):
    ...
    if exp_info['Articles'].isna().sum() == 0:
        ...
        exp_info_firstlines = exp_info.groupby(
            'Articles').first().reset_index()
        exp_info_firstlines['x'] = exp_info.groupby(
            'Articles')['x'].apply(list).values
        exp_info_firstlines['y'] = exp_info.groupby(
            'Articles')['y'].apply(list).values
        exp_info_firstlines['z'] = exp_info.groupby(
            'Articles')['z'].apply(list).values
        ...
        exp_info_firstlines['Coordinates_mm'] = exp_info_firstlines['Coordinates_mm'].astype(
            object)
...
def convert_tal_2_mni(exp_info):
    ...
    exp_info.loc[exp_info['CoordinateSpace'] == "TAL",
                 "Coordinates_mm"] = exp_info[exp_info['CoordinateSpace'] == "TAL"].apply(
...
def calculate_gaussian_width(exp_info):
    ...
    subject_uncertainty = (11.6/(2*np.sqrt(2/np.pi)) *
                           np.sqrt(8*np.log(2))) / np.sqrt(exp_info['Subjects'])
    exp_info['Smoothing'] = np.sqrt(
        template_uncertainty**2 + subject_uncertainty)
...
def create_tasks_table(exp_info):
    ...
    task_names, task_counts = np.unique(
        np.hstack(exp_info.Tags), return_counts=True)
    ...
    tasks = tasks.sort_values(
        by='Num_Exp', ascending=False).reset_index(drop=True)
...
def read_experiment_info(filename):
    ...
    exp_info = exp_info[['Articles', 'Subjects',
                         'CoordinateSpace', 'Tags', 'NumberOfFoci', 'Coordinates']]
