...
from utils.template import shape, prior, affine
...
def compute_hx(ma, bin_edges):
    ...
    for i in range(ma.shape[0]):
        ...
        bin_idxs, counts = np.unique(np.digitize(
            data[prior], bin_edges), return_counts=True)
...
def compute_clusters(z, cluster_thresh, cut_cluster=None):
    # Threshold z-values based on the specified cluster threshold
    sig_arr = (z > norm.ppf(1 - cluster_thresh)).astype(int)
    ...
    if cut_cluster:
        significant_clusters = np.bincount(labels[labels > 0]) > cut_cluster
...
def compute_null_ale(sample_space, num_foci, kernels):
    null_foci = np.array([sample_space[:, np.random.randint(
        0, sample_space.shape[1], num_peak)].T for num_peak in num_foci],
        dtype=object)
...
def compute_null_cutoffs(sample_space, num_foci, kernels, step=10000,
                         cluster_thresh=0.001, target_n=None,
    ...
    _, null_max_cluster = compute_clusters(null_z, cluster_thresh)
    null_max_tfce = 0
    if tfce_enabled:
...
def compute_sub_ale(sample, ma, hx, bin_centers,
                    cut_cluster, step=10000, thresh=0.001):
    hx_conv = compute_hx_conv(hx, bin_centers, step)
    ale = compute_ale(ma[sample])
    z = compute_z(ale, hx_conv, step)
    z, max_cluster = compute_clusters(z, thresh, cut_cluster=cut_cluster)
    z[z > 0] = 1
    return z
