...
def contribution(project_path, exp_df, exp_name, tasks, tfce_enabled=True):
    ...
    for corr_method in corr_methods:
        ...
        results = loadsave.load(
            project_path
            / f"Results/MainEffect/Full/Volumes/{exp_name}_{corr_method}05.nii"
        ...
        if results.any() > 0:
            ...
            for index, label in enumerate(np.argsort(count)[::-1][1:]):
                ...
                center = np.median(
                    np.dot(
                        MNI_AFFINE,
                        np.pad(cluster_idxs, ((0, 1), (0, 0)), constant_values=1),
                    ),
                    axis=1,
                )
                if cluster_idxs[0].size > 5:
                    ...
                    for idx in exp_idxs:
                        ...
                        contribution_arr[idx, 1] = (
                            100 * np.sum(ma_cluster_mask[idx]) / cluster_size
                        )  # noqa
                        proportion_of_ale = (
                            compute_ale(np.delete(ma_cluster_mask, idx, axis=0))
                            / ale_cluster_mask
                        )
                    ...
                    contribution_arr[:, 2] = (
                        contribution_arr[:, 2] / np.sum(contribution_arr[:, 2]) * 100
                    )  # noqa
                    exp_idxs_sorted_by_contrib = np.argsort(contribution_arr[:, 2])[
                        ::-1
                    ]  # noqa

                    for idx in exp_idxs_sorted_by_contrib:
                        if (
                            contribution_arr[idx, 2] > 0.1
                            or contribution_arr[idx, 3] > 5
                        ):
                            stx = list(" " * (exp_df.Articles.str.len().max() + 2))
                            stx[0 : len(exp_df.Articles[idx])] = exp_df.Articles[idx]
                            stx = "".join(stx)
