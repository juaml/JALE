...
from core.analyses.contrast import contrast, balanced_contrast
...
for row_idx in range(analysis_df.shape[0]):
    ...
    if analysis_df.iloc[row_idx, 0] == "C":  # Contrast Analysis
        ...
        if len(exp_idxs1) < 17 or len(exp_idxs2) < 17:
            print("One of two effects selected for contrast"
                  "contains less than 17 Experiments."
                  "Please interprete results carefully!")
        for dataset_idx in [0, 1]:
            if not Path(project_path /
                        f"Results/MainEffect/Full/Volumes/{meta_names[dataset_idx]}_cFWE05.nii").exists():
                main_effect(project_path,
                            exp_dfs[dataset_idx],
        ...
        exp_dfs = [exp_all_df.loc[exp_idxs1].reset_index(drop=True),
                   exp_all_df.loc[exp_idxs2].reset_index(drop=True)]
    ...
    if analysis_df.iloc[row_idx, 0][0] == "B":  # Balanced Contrast Analysis:
        ...
        for dataset_idx in [0, 1]:
            if not Path(project_path /
                        f"Results/MainEffect/CV/Volumes/{meta_names[dataset_idx]}_{target_n}.nii").exists():
                main_effect(project_path,
                            exp_dfs[dataset_idx],
                            meta_names[dataset_idx],
                            target_n=target_n,
        ...
        balanced_contrast(project_path,
                          exp_dfs,
                          meta_names,
                          target_n,
