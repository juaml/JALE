...
class AddAnalysisWindow(customtkinter.CTkToplevel):
    ...
    def create_widgets(self):
        self.analysis_type_label = customtkinter.CTkLabel(
        ...
        self.analysis_name_label = customtkinter.CTkLabel(
            self, text="Analysis Name")
        ...
        self.tag1_label.grid(row=1, column=2, padx=10,
                             pady=(10, 0), sticky='w')
        self.tag1 = customtkinter.CTkOptionMenu(
            self,
            values=self.task_df.Name.values,
            command=lambda value: self.tag_enable_add_button(group=1,
        ...
        # Experiment Group2
        self.second_group_label = customtkinter.CTkLabel(
            self, text="2nd Effect")
        ...
        self.tag1_group2 = customtkinter.CTkOptionMenu(
            ...
            state='disabled',
            command=lambda value: self.tag_enable_add_button(group=2,
                                                             selected_value=value))
        self.tag1_group2.grid(row=3, column=2, padx=10, pady=10, sticky='w')

        self.add_tag_button_group2 = customtkinter.CTkButton(
            self, text='+', command=lambda: self.add_tag_button_event(group=2), state='disabled')
        ...
        self.add_analysis_button.grid(
            row=5, column=3, padx=10, pady=10, sticky='w')

        self.import_analysis_file_button = customtkinter.CTkButton(
            master=self, text='Import Analysis', command=self.import_analysis_file_button_event)
        self.import_analysis_file_button.grid(
    ...
    def clear_group_widgets(self, group):
        ...
        for widget in widgets:
            if isinstance(widget, customtkinter.CTkOptionMenu) and widget not in [self.analysis_type,
                                                                                  self.tag1,
                                                                                  self.tag1_group2]:
                widget.destroy()
    ...
    def update_group2_state(self, value):
        state = 'normal' if value in [
            "Standard Contrast", "Balanced Contrast"] else 'disabled'
    ...
    def add_tag_button_event(self, group):
        ...
        logic_label = customtkinter.CTkLabel(
            self, text=f"Logic Operator {tag_count}")
        logic_label.grid(row=1, column=current_column,
                         padx=10, pady=(10, 0), sticky='w')
        logic_menu = customtkinter.CTkOptionMenu(
            self, values=['and', 'or', 'not'])
        logic_menu.grid(
            row=info['row'], column=current_column, padx=10, pady=10, sticky='w')

        tag_label = customtkinter.CTkLabel(self, text=f"Tag {tag_count + 1}")
        tag_label.grid(row=1, column=current_column + 1,
                       padx=10, pady=(10, 0), sticky='w')
    ...
    def add_analysis_button_event(self):
        ...
        if self.analysis_type.get() in ["Standard Contrast", "Balanced Contrast"]:
            ...
            for i in range(1, self.tag_count_group2):
                logic = self.grid_slaves(
                    row=3, column=3 + (i - 1) * 2)[0].get()
