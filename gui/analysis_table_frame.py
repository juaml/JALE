...
class AnalysisTableFrame(customtkinter.CTkFrame):
    def __init__(self, master):
        ...
        self.analysis_type_abbreviation_dict = {
            'Main Effect': 'M', 'IPA': 'P', 'Standard Contrast': 'C', 'Balanced Contrast': 'B'}
        ...
        self.tree['columns'] = (
            "Analysis Type", "Analysis Name", "Group 1 Logic", "Group 2 Logic")

        self.tree.column("#0", width=0, stretch=tk.NO)
        self.tree.column("Analysis Type", anchor=tk.CENTER,
                         width=100,  stretch=tk.NO)
        self.tree.column("Analysis Name", anchor=tk.CENTER,
                         width=250,  stretch=tk.NO)
        self.tree.column("Group 1 Logic", anchor=tk.CENTER,
                         width=280,  stretch=tk.NO)
        self.tree.column("Group 2 Logic", anchor=tk.CENTER,
                         width=280,  stretch=tk.NO)

        self.tree.heading("#0", text="", anchor=tk.CENTER)
        self.tree.heading(
            "Analysis Type", text="Analysis Type", anchor=tk.CENTER)
        self.tree.heading(
            "Analysis Name", text="Analysis Name", anchor=tk.CENTER)
        self.tree.heading(
            "Group 1 Logic", text="Group 1 Logic", anchor=tk.CENTER)
        self.tree.heading(
            "Group 2 Logic", text="Group 2 Logic", anchor=tk.CENTER)
    ...
    def fill_table(self, analysis_df):
        ...
        for row in analysis_df.itertuples(index=False):
            ...
            self.tree.insert("", tk.END, values=(
                analysis_type, row[1], row[2], row[3]))
    ...
    def format_imported_analysis_file(self, analysis_df):
        ...
        for row in range(analysis_df.shape[0]):
            ...
            if analysis_type in ['B', 'C']:
                ...
                new_row = pd.DataFrame([{'analysis_type': analysis_type, 'analysis_name': analysis_name,
                                       'group1_logic': group1_logic, 'group2_logic': group2_logic}])
                skip_row = True
            else:
